      ...
      module Eval
        ...
        def self.run_eval(options, adapter_file)
          ...
          begin
            ...
            # Check for differences in compare mode
            ...
            if expected
              ...
              # No expected value - just show output
            ...
            # Build final spec data for saving
            ...
            # Build final spec data with error
            final_spec = build_final_spec(
              name: name,
              hint: hint,
              complexity: complexity,
        ...
        def self.print_spec_header(template, name, hint, complexity, assigns, verbose)
          ...
          # Environment if present and verbose
        ...
        def self.print_pass(actual, compare_mode, reference_output)
        ...
        def self.print_fail(expected, actual, hint, spec_data)
          ...
          # Show character diff for similar strings
          ...
          # Only show hint if it wasn't already shown (i.e., if it came from spec_data)
          # Since we always show hint in header now, don't repeat it
        end

        def self.print_output_only(actual)
          puts "\e[36mOutput:\e[0m"
          print_indented(actual)
        end

        def self.print_error(error, hint, verbose, spec_data)
          ...
          # Only show hint if it wasn't already shown in header
          puts ""
        end

        def self.print_output_value(value)
          if value.to_s.empty?
        ...
        def self.load_spec_from_string(yaml_content, options)
          require "yaml"
        ...
        def self.build_final_spec(name:, hint:, complexity:, template:, assigns:, expected:, passed:, error: nil)
          spec = {}
          spec["name"] = name
          ...
          if error
            # Record error using the errors: format with substring patterns
        ...
        # Parse error message into substring patterns for flexible matching
        # Input: "Liquid syntax error (line 1): Syntax Error in tag 'if' - Valid syntax: if [expression]"
        # Output: ["Liquid syntax error", "1", "Syntax Error in tag 'if' - Valid syntax: if [expression]"]
        def self.parse_error_patterns(message)
          # Try to match: "ErrorType (line N): message"
        ...
        def self.append_to_daily_file(spec_data, has_difference)
          require "yaml"
          require "date"

          daily_file = "/tmp/liquid-spec-#{Date.today}.yml"

          # Clean up spec_data for output
          ...
          # Read existing specs or start fresh
          existing_specs = []
        ...
        def self.show_contribution_message(has_difference)
