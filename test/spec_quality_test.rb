# frozen_string_literal: true

require "liquid"
require "set"
require_relative "test_helper"

class SpecQualityTest < Minitest::Test
  EARLY_HINT_CEILING = 220
  RESOURCE_LIMIT_FLOOR = 500
  COMPLEXITY_CEILING = 1000

  def all_specs
    Liquid::Spec::Suite.all.flat_map do |suite|
      Liquid::Spec::SpecLoader.load_suite(suite).map { |spec| [suite, spec] }
    end
  end

  def test_complexity_scores_do_not_exceed_ceiling
    offenders = all_specs.filter_map do |suite, spec|
      complexity = spec.complexity || suite.minimum_complexity || COMPLEXITY_CEILING
      next if complexity <= COMPLEXITY_CEILING

      "#{spec.source_file}:#{spec.line_number}: #{spec.name} complexity=#{complexity}"
    end

    assert_empty offenders, "Complexity scores must be <= #{COMPLEXITY_CEILING}:\n#{offenders.join("\n")}"
  end

  def test_specs_through_early_core_ramp_have_effective_hints
    offenders = all_specs.filter_map do |suite, spec|
      complexity = spec.complexity || suite.minimum_complexity || COMPLEXITY_CEILING
      next if complexity > EARLY_HINT_CEILING
      next unless spec.effective_hint.to_s.strip.empty?

      "#{spec.source_file}:#{spec.line_number}: [#{complexity}] #{spec.name}"
    end

    assert_empty offenders, "Specs with complexity <= #{EARLY_HINT_CEILING} need hints:\n#{offenders.join("\n")}"
  end

  # Specs at complexity <= 220 must carry a spec-LEVEL hint (spec.hint), not
  # just the suite/file boilerplate (effective_hint). Existing generated/bulk
  # specs without one are grandfathered via test/spec_hint_baseline.txt; any
  # NEW spec at c<=220 lacking a spec-level hint is a regression.
  SPEC_HINT_BASELINE_PATH = File.expand_path("spec_hint_baseline.txt", __dir__)

  def spec_hint_baseline
    @spec_hint_baseline ||= File.exist?(SPEC_HINT_BASELINE_PATH) ?
      File.readlines(SPEC_HINT_BASELINE_PATH).map { |l| l.chomp }.to_set :
      Set.new
  end

  def test_specs_through_220_have_spec_level_hints
    root = File.expand_path("..", __dir__)
    current = all_specs.filter_map do |suite, spec|
      complexity = spec.complexity || suite.minimum_complexity || COMPLEXITY_CEILING
      next if complexity > EARLY_HINT_CEILING
      next unless spec.hint.to_s.strip.empty?
      rel = spec.source_file.to_s.sub(root + "/", "")
      "#{rel}\t#{spec.name}"
    end.to_set

    baseline = spec_hint_baseline
    new_offenders = current - baseline
    stale = baseline - current

    unless new_offenders.empty?
      flunk "Specs with complexity <= #{EARLY_HINT_CEILING} need a spec-level hint (spec.hint), not just suite/file boilerplate. Add a hint to each new spec below:\n#{new_offenders.sort.join("\n")}"
    end

    unless stale.empty?
      flunk "test/spec_hint_baseline.txt has stale entries (specs that now HAVE a spec-level hint or were removed). Remove them from the baseline, or regenerate it with: ruby -Ilib scripts/generate_spec_hint_baseline.rb\n#{stale.sort.join("\n")}"
    end
  end

  def test_resource_limit_specs_do_not_appear_in_beginner_ramp
    offenders = all_specs.filter_map do |suite, spec|
      next if spec.raw_resource_limits.nil? || spec.raw_resource_limits.empty?

      complexity = spec.complexity || suite.minimum_complexity || COMPLEXITY_CEILING
      next if complexity >= RESOURCE_LIMIT_FLOOR

      "#{spec.source_file}:#{spec.line_number}: [#{complexity}] #{spec.name}"
    end

    assert_empty offenders, "Resource-limit specs should be complexity >= #{RESOURCE_LIMIT_FLOOR}:\n#{offenders.join("\n")}"
  end
end
